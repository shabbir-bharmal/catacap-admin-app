...
router.put("/restore", async (req: Request, res: Response) => {
  try {
    ...
    const usersResult = await pool.query(
      `SELECT id, email, deleted_at, deleted_by FROM users WHERE id = ANY($1) AND is_deleted = true`,
      [ids]
    );
    ...
    try {
      ...
      const tableMetaCache = new Map<
        string,
        { exists: boolean; hasIsDeleted: boolean; hasDeletedAt: boolean; hasDeletedBy: boolean }
      >();
      const tableMeta = async (table: string) => {
        ...
        const meta = { exists, hasIsDeleted, hasDeletedAt, hasDeletedBy };
        tableMetaCache.set(table, meta);
        return meta;
      };

      // Process each user separately so the restore is scoped to that user's
      // own delete event (matching the user's deleted_at and deleted_by). This
      // avoids resurrecting records that were already soft-deleted before the
      // user was deleted.
      for (const user of deletedUsers) {
        ...
        const campaignIds = await collectIdsScoped(
          "campaigns",
          `user_id = $1`,
          [userId]
        );
        ...
        const groupIds = await collectIdsScoped(
          "groups",
          `owner_id = $1`,
          [userId]
        );
