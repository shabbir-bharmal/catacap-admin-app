...
namespace Invest.Controllers
{
    ...
    public class RecommendationsController : ControllerBase
    {
        ...
        public RecommendationsController(RepositoryContext context, IRepositoryManager repositoryManager, IMapper mapper, IMailService mailService, IHttpContextAccessor httpContextAccessors, EmailQueue emailQueue, ImageService imageService, AppSecrets appSecrets)
        {
            _context = context;
        ...
        private async Task AddPersonalDeduction(User user, decimal amount, string investmentName, int? campaignId, string? grantType)
        {
            ...
            {
                UserId = user.Id,
                PaymentType = !string.IsNullOrWhiteSpace(grantType) ? isAdmin ? $"{grantType}, {loginUser.UserName!.Trim().ToLower()}" : grantType : "Manually",
